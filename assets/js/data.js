...
// подтверждены для bybit/bitget/kucoin; okx/mexc/gate добавлены в расчёт
...
//
// ВАЖНО: индустрия пропфирм не регулируется государством. Отбор — в два
// уровня (полный текст на /prop-firms/ и /red-flags/):
// 1) Жёсткое «нет» без исключений: фирма (или связанное юрлицо) в
//    официальном списке предупреждений/чёрном списке регулятора (Банк
//    России, SEC, FCA и т.п.), санкции, подтверждённый систематический
//    паттерн жалоб на невыплаты, ретроактивные изменения правил для уже
//    профинансированных трейдеров, отсутствие проверяемой юр. регистрации.
// 2) Ориентиры надёжности (можно не выполнять полностью — тогда
//    `includeNote` объясняет, что именно не дотягивает): Trustpilot 4.0+
//    при достаточном числе отзывов, многолетняя история выплат (или
//    молодая фирма без нареканий по жёстким пунктам), в идеале —
//    поддержка регулируемого брокера. Это не финансовая лицензия и не
//    госаккредитация.
//
// Отдельно от безопасности — практическая проверка релевантности для
// аудитории сайта: доступна ли фирма резидентам РФ/СНГ вообще. Многие
// американские фьючерсные пропфирмы (Topstep, и почти все конкуренты —
// Apex Trader Funding, Earn2Trade, MyFundedFutures — проверено 2026-09-14)
// соблюдают ограничения OFAC и прямо исключают Россию из списка
// разрешённых юрисдикций — регистрация и выплаты для резидентов РФ у них
// закрыты независимо от остальных показателей фирмы. Не безопасность, а
// применимость: такую фирму можно оставить в сравнении (как Topstep, с
// явным предупреждением в `includeNote` и на странице фирмы), но не стоит
// молчать об этом ограничении.
//
...
const PROP_FIRMS = [
  ...
  },
  {
    ...
    brokerBacking: null,
    officialUrl: "https://www.topstep.com",
    includeNote: "Резиденты России фактически не могут торговать в Topstep — как и большинство американских фьючерсных пропфирм, Topstep соблюдает ограничения OFAC, и Россия у них в списке недоступных юрисдикций. Также рейтинг Trustpilot ниже наших ориентиров, но это одна из старейших фьючерсных пропфирм (с 2012) с многолетней публичной историей выплат. Оценивайте осторожно и читайте свежие отзывы.",
    dataVerified: false,
